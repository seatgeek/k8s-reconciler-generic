...
type Logic[S Subject, C any] interface {
	...
	// AugmentObjectMeta is called to add Logic-specific labels, annotations, etc to derived objects.
	AugmentObjectMeta(context *Context[S, C], tier, suffix string, meta kmetav1.ObjectMeta) kmetav1.ObjectMeta
...
func (c *Context[_, _]) ObjectMeta(tier, suffix string) kmetav1.ObjectMeta {
	...
	labels := map[string]string{
		...
		c.Owner.LabelKey("component"): name,
	}
	if tier != "" {
		labels[c.Owner.LabelKey("tier")] = tier
	}
	return c.Owner.Logic.AugmentObjectMeta(c, tier, suffix, kmetav1.ObjectMeta{
		Name:      name,
